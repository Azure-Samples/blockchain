...
data class NumericParams(
    val intParam: Int = 1, val longParam: Long = 1L, val floatParam: Float = 1.0f,
...
class Functions(val observer: StringBuilder = StringBuilder()) {
    ...
    fun twoParams(name: String?, age: Int = 21) {
        observer.append("twoParams: $name,$age")
    }
    ...
    // testing custom resolver on function params
    fun resolvedClassParam(party: Party) {
        observer.append("resolvedClassParam: $party")
    }

    // testing custom resolver fires WITHIN the class passed as a param, so
    // this is to test that recursive rules will fires
    fun resolvedClassWithParam(idParam : UniqueIdentifierParam){
        observer.append("resolvedClassWithParam: $idParam")
    }

}

class TestTokenisedAsset : TokenizableAssetInfo {
    override val displayTokenSize: BigDecimal
        get() =  BigDecimal.ONE.scaleByPowerOfTen(-3)
