...
namespace Workbench.Client
{
	public sealed class GatewayApi
	{
        ...
        public int CONTRACTS_TOP_QUERY_PARAM_DEFAULT = 5;
        public int CONTRACTS_TOP_QUERY_PARAM = 5;
        public bool HAS_MORE_CONTRACTS = false;
...
#region WORKFLOWINSTANCES (CONTRACTS)
        ...
        {
            // Using a different TOP Query parameter because the mobile sample implements polling and needs to constantly poll for updates
            var result = await getDataObjectFromAPI<WorkflowInstancesReturnType>($"{BaseUrl}contracts?workflowid={workflowInstanceID}&top={CONTRACTS_TOP_QUERY_PARAM}&skip={skip}&sortBy={sortBy}");
            if (result != null)
            {
                if (result.Contracts.Count <= CONTRACTS_TOP_QUERY_PARAM && result.NextLink.Contains($"skip={CONTRACTS_TOP_QUERY_PARAM}"))
                {
                    HAS_MORE_CONTRACTS = true;
                }
                else
                {
                    HAS_MORE_CONTRACTS = false;
                }
            }
